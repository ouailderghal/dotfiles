...
** Font configuration
Make sure that you have [[https://github.com/tonsky/FiraCode][Fira Code]] font installed on your system (I use Arch BTW!).
...
#+begin_src emacs-lisp
  (setq od/default-fixed-font "Fira Code")
...
Change cursor shape to /box/ on insert mode.
...
** Open file manager
Open =Thunar= file manager from within Emacs.

#+begin_src emacs-lisp
  (defun od/open-thunar ()
    "Open simple terminal in home directory."
    (interactive)
    (shell-command "thunar ~"))

  (defun od/open-thunar-here ()
    "Open simple terminal in current directory."
    (interactive)
...
#+begin_src emacs-lisp
  (global-set-key (kbd "<f9>") 'od/open-thunar)
  (global-set-key (kbd "S-<f9>") 'od/open-thunar-here)
#+end_src
